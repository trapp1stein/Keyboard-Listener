...
func extract_event(inbuf []byte) (int, KeyEvent) {
	...
	if inbuf[0] == '\033' {
		...
		// kombinasyondan kurtuluş yapılır
		if size, event := parse_escape_sequence(inbuf); size != 0 {
			return size, event
		} else {
			// escape kaçışı
			i := 1 // çoklu escape teke düşürelim.
...
// olayı bekleyelim, ardından fonksiyonu çağırıyoruz
